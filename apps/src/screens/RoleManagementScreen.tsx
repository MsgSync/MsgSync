  ...
  return (
    <div className="flex w-full flex-col space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[300px_1fr]">
        <section className="rounded-lg border border-[#3d494c] bg-[#181c24] p-3"><div className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-wider text-[#869397]">Platform roles</div><div className="space-y-1">{roles?.map(role => <button key={role.role} type="button" onClick={() => setSelectedRole(role.role)} className={`w-full rounded-lg p-3 text-left transition-colors ${selected?.role === role.role ? 'border border-[#06b6d4]/50 bg-[#06b6d4]/10' : 'border border-transparent hover:bg-[#262a33]'}`}><div className="flex items-center justify-between"><span className={`text-[13px] font-semibold ${selected?.role === role.role ? 'text-[#4cd7f6]' : 'text-[#dfe2ee]'}`}>{role.label}</span>{user?.role === role.role && <span className="rounded bg-[#4edea3]/15 px-1.5 py-0.5 text-[9px] text-[#4edea3]">YOU</span>}</div><p className="mt-1 text-[10px] leading-4 text-[#869397]">{role.description}</p><div className="mt-2 font-code-metric text-[9px] text-[#68767b]">{role.permissions.length} PERMISSIONS</div></button>)}</div></section>
        <section className="rounded-lg border border-[#3d494c] bg-[#1c2028] p-5 shadow-sm"><div className="flex items-start gap-3 border-b border-[#293240] pb-4"><span className="material-symbols-outlined text-[24px] text-[#4cd7f6]">admin_panel_settings</span><div><h2 className="text-[16px] font-semibold text-[#dfe2ee]">{selected?.label || 'Role'} permissions</h2><p className="mt-1 text-[11px] text-[#869397]">{selected?.description}</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(PERMISSION_GROUPS).map(([permission, label]) => { const enabled = selected?.permissions.includes(permission); return <div key={permission} className={`flex items-center gap-2 rounded border px-3 py-2 text-[11px] ${enabled ? 'border-[#4edea3]/20 bg-[#4edea3]/5 text-[#dfe2ee]' : 'border-[#293240] bg-[#12161f] text-[#536066]'}`}><span className={`material-symbols-outlined text-[16px] ${enabled ? 'text-[#4edea3]' : 'text-[#3d494c]'}`}>{enabled ? 'check_circle' : 'remove_circle'}</span><span>{label}</span></div>; })}</div><div className="mt-5 flex items-start gap-2 rounded-lg border border-[#3d494c] bg-[#0a0e16] p-3 text-[10px] leading-5 text-[#869397]"><span className="material-symbols-outlined text-[16px] text-[#4cd7f6]">info</span>Roles are centrally managed. Assign users from User Management; API permissions and tenant scope update automatically.</div></section>
      </div>
    </div>
  );
};
